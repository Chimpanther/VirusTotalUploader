...
namespace uploader
{
    public partial class UploadForm : DarkForm
    {
        ...
        public UploadForm(MainForm mainForm, Settings settings, bool reopen, string[] paths)
        {
            _paths = paths;
            _mainForm = mainForm;
            _settings = settings;
        ...
        private void Upload()
        {
            ...
            foreach (var p in _paths)
            {
                ...
                {
                    _filesToUpload.AddRange(Directory.GetFiles(p, "*.*", SearchOption.AllDirectories));
                }
        ...
        {
            if (_paths.Length == 1 && !Directory.Exists(_paths[0]))
            {
